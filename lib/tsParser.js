    ...
    exportedConstructs.forEach(function(node) {
      if (node.kindString === 'Class' ||
          ...
          node.kindString === 'Type alias') {
            ...
            createAnchor(node);
            var title = tsHelpers.getNodeTitle(node);
        ...
        if ((node.kindString === 'Class' || node.kindString === 'Interface' ||
          ...
          children.forEach(function(child) {
            if (child.inheritedFrom || child.flags.isPrivate || child.flags.isProtected) {
             ...
             // This is needed in UI, good to keep the eligibility logic at one place
              ...
              createAnchor(child);
              var title = tsHelpers.getNodeTitle(child);
              this.sections.push({'title': title, 'anchor': child.anchorId, 'depth': 4});
            }
          }.bind(this));
        }
      }
