...
tsHelpers.getVariableStr = function(varNode) {
  var kind = tsHelpers.getVariableType(varNode);
  var type = tsHelpers.getTypeStr(varNode.type);
  var defaultVal = varNode.defaultValue;
  var str = kind + ' ' + varNode.name;
  var str = type? str + ': ' + type: str;
...
tsHelpers.getNodeTitle = function(node) {
  ...
  if (kind === 'Variable' || kind == 'Property') return node.name;
