...
function Docs(config) {
  ...
  this.tsTemplates = {
      classTemplate : fs.readFileSync(TS_CLASS_TEMPLATE, 'utf8'),
      interfaceTemplate : fs.readFileSync(TS_INTERFACE_TEMPLATE, 'utf8'),
      functionTemplate : fs.readFileSync(TS_FUNCTION_TEMPLATE, 'utf8')
  };
}
  ...
  te.on('done', function () {
    matchedFiles.forEach(function (f) {
      ...
      if(typeof f === 'object') {
        ...
        contents = files[f];
        if(this.hasExt(f)) {       
          var doc = new Doc(f, contents, path.extname(f) === '.js', path.extname(f) === '.ts', self);
