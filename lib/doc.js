...
Doc.prototype.parseTypeScript = function () {
  ...
  if(!project){
    ...
    fileNodes.forEach(function(node){
        if(node.kindString === 'Class' || node.kindString === 'Interface' || node.kindString === 'Function'){
          this.classes.push(new TSConstruct(node, this)); 
          var anchorId = node.kindString+node.name+node.id;
          node.anchorId = anchorId;
          this.sections.push({'title': node.kindString + ': ' + node.name, 'anchor': anchorId, 'depth': 3});  
          // build sections for children
          const children = node.children;
          if((node.kindString === 'Class' || node.kindString === 'Interface') && children && children.length > 0){
              children.forEach(function(child){
                  if(
                    (child.kindString === 'Property' && !child.flags.isPrivate) ||
                    (child.kindString === 'Constructor') ||
                    (child.kindString === 'Method' && !child.flags.isPrivate)
                    ){
                      var anchorId = child.kindString+child.name+child.id;
                      child.anchorId = anchorId;
                      this.sections.push({'title': child.name, 'anchor': anchorId, 'depth': 4});
                  }
              }.bind(this));
          }       
        }
